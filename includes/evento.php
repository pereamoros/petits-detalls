<?php
    ...
    if($event_trigger) {
        ?>
            <style>
                .event,.modal-content{text-align:center;color:var(--bg-color)}.event{display:none;padding:1rem 2.5rem;background-color:var(--primary-color)}.close{position:absolute;right:2.5rem;font-size:28px;line-height:1;cursor:pointer}.modal{display:none;position:fixed;z-index:9999;left:0;top:0;width:100%;height:100%;padding:4rem;overflow:auto;background-color:rgba(0,0,0,.5);backdrop-filter:blur(10px)}.modal-content{display:grid;grid-template-rows:auto 1fr;place-content:center;gap:2rem;position:relative;font-size:var(--l-font-size);height:100%}.modal-img{background-image:url('/images/<?=$event_img?>');background-size:contain;background-position:center;background-repeat:no-repeat}
            </style>
            <div class="event">
